...
export default function PostUserInfoPopup({ avatar, name, description, username }: UserType) {
  return (
    <div className='bg-white w-80 h-40 absolute top-[-10.75rem] shadow-xl p-4 rounded-2xl flex flex-col gap-2 z-10 border-[1px] border-solid border-[#cacaca]'>
      ...
      <div className='flex items-center gap-3'>
        ...
        <div className='flex flex-col'>
          <h2 title={name} className='text-base tracking-wide leading-6 font-semibold'>
            {name}
          </h2>
          <span title={`@${username}`} className='text-xs tracking-wide font-thin text-[#757575]'>{`@${username}`}</span>
        </div>
      </div>
      <hr />
      <p className='text-sm tracking-wide font-thin text-[#757575] h-20 overflow-hidden text-ellipsis'>{description}</p>
    </div>
  )
}
