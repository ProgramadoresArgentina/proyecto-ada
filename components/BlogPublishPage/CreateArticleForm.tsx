...
export default function CreateArticleForm(state) {
    ...
    const publishBlog = () => {
        ...
        const endpoint = state.id ? `/api/articles/put/${state.id}` : '/api/articles/post';
        fetch(endpoint, {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                ...
                setLoading(false);
                launchToast("success", state.id ? "Blog actualizado" : "Blog creado correctamente");
